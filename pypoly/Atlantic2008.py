...
COMMUNITY_CHEST_CARDS = [
  CommunityChestCard(lambda player, card: AdvanceTo(player=player, card=card, destination=myPlace.resolve('Go')),
    "Advance to Go (Collect $200) ",
    "Mr. M strides in 7-league boots"),
  CommunityChestCard(lambda player, card: GetFromBank(player=player, card=card, amount=200),
    "Bank error in your favor - Collect $200",
    "Mr. M falls back in astonishment as an arm presents a sheaf of cash out of a bank teller's window,"),
  CommunityChestCard(lambda player, card: PayToBank(player=player, card=card, amount=50),
    "Doctor's fees {fee} - Pay $50",
    "Mr. M angrily brandishes crutches as he stomps with a leg cast"),
  CommunityChestCard(lambda player, card: GetFromBank(player=player, card=card, amount=50), 
    "From sale of stock you get $50", #{$45} 
    "Mr. M, happily entangled in the tape of a stock ticker, waves cash (with no $ sign this time)"),
  CommunityChestCard(lambda player, card: GetJailFree(player=player, card=card, is_chance=False),
  ...
  CommunityChestCard(lambda player, card: GoToJail(player=player, card=card),
    "Go to Jail - Go directly to jail - Do not pass Go - Do not collect $200",
    "A truncheon-wielding policeman in a light-colored uniform lifts a surprised Mr M by the collar"),

  CommunityChestCard(lambda player, card: CollectFromAll(player=player, card=card, amount=50),
  ...
  CommunityChestCard(lambda player, card: GetFromBank(player=player, card=card, amount=100),
    "Holiday {Xmas} Fund matures - Receive {Collect} $100",
    "Mr. M carries along a giant Xmas sock containing a sheaf of cash"),
  CommunityChestCard(lambda player, card: GetFromBank(player=player, card=card, amount=20),
    "Income tax refund - Collect $20",
    "Mr M faints back against a man displaying the Refund paper"),
  CommunityChestCard(lambda player, card: CollectFromAll(player=player, card=card, amount=10),
  ...
  CommunityChestCard(lambda player, card: PayToBank(player=player, card=card, amount=100),
  ...
  CommunityChestCard(lambda player, card: PayToBank(player=player, card=card, amount=150),
    "Pay school fees {tax} of $150",
    "A bespectacled schoolboy unhappily receives a head pat and a dime ((Rockefeller style) from Mr. M."),
  CommunityChestCard(lambda player, card: GetFromBank(player=player, card=card, amount=25),
  ...
  CommunityChestCard(lambda player, card: Repair(player=player, card=card, house=40, hotel=115),
    """You are assessed for street repairs - $40 per house - $115 per hotel""",
    """Mr. M., supported by his near-ubiquitous cane in his left hand, 
    holds a pick and shovel over his right shoulder"""),
  CommunityChestCard(lambda player, card: GetFromBank(player=player, card=card, amount=10),
...
CHANCE_CARDS = [
  ChanceCard(lambda player, card: AdvanceTo(player=player, card=card, destination=myPlace.resolve('Go')),
    "Advance to Go (Collect $200)",
    "Mr. M hops on both feet."),
  ChanceCard(lambda player, card: AdvanceTo(player=player, card=card, destination=myPlace.resolve("Illinois Avenue")),
  ...
  ChanceCard(lambda player, card: AdvanceTo(player=player, card=card, destination=myPlace.resolve("St. Charles Place")),
    "Advance to St. Charles Place - If you pass Go, collect $200",
    "Mr. M hurries along, hauling a little boy by the hand"),
  ChanceCard(lambda player, card: AdvanceToNearestUtility(player=player, card=card),
  ...
  ChanceCard(lambda player, card: AdvanceToNearestRailroad(player=player, card=card),
  ...
  ChanceCard(lambda player, card: AdvanceToNearestRailroad(player=player, card=card),
    ...
    """At lower left, Mr. M carries a large flatiron token with two hands;
    at upper right a railroad crossing is seen"""),
  ChanceCard(lambda player, card: GetFromBank(player=player, card=card, amount=50),
    "Bank pays you dividend of $50",
    """With his feet up on a telephone-bearing desk, 
    Mr. M leans back in an overstuffed chair, blowing cigar smoke rings"""),
  ChanceCard(lambda player, card: GetJailFree(player=player, card=card, is_chance=True),
  ...
  ChanceCard(lambda player, card: Retreat(player=player, card=card, amount=3),
    "Go back 3 spaces",
    "Mr. M is hauled back by a cane hooked around his neck"
    # {Presumably an allusion to amateur nights at theaters}
    ),
  ChanceCard(lambda player, card: GoToJail(player=player, card=card),
    "Go to Jail - Go directly to Jail - Do not pass Go, do not collect $200",
    "A truncheon-carrying policeman in a dark-colored uniform hauls a surprised Mr M along by the feet"),
  ChanceCard(lambda player, card: Repair(player=player, card=card, house=25, hotel=100),
  ...
  ChanceCard(lambda player, card: PayToBank(player=player, card=card, amount=15),
    "Pay poor tax of $15",
  ...
  ChanceCard(lambda player, card: AdvanceTo(player=player, card=card, destination=myPlace.resolve('RR1')),
  ...
  ChanceCard(lambda player, card: AdvanceTo(player=player, card=card,destination=myPlace.resolve("Boardwalk")),
  ...
  ChanceCard(lambda player, card: PayToAll(player=player, card=card, amount=50),
    """You have been elected Chairman of the Board - Pay each player $50""",
    """A newsboy shouts an Extra with Mr. M's headshot on its front page"""),
  ChanceCard(lambda player, card: GetFromBank(player=player, card=card, amount=150),
  ...
  ChanceCard(lambda player, card: GetFromBank(player=player, card=card, amount=100),
